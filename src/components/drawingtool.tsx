...
const DrawingTool = () => {
  ...
  useLayoutEffect(() => {
    ...
    elements.forEach((ele:any) => rc.draw(ele.roughEle));
  }, [elements]);

  const startDrawing = (event:any) => {
    setDrawing(true);
    const [clientX, clientY ] : [any, any] = event;
    const newEle : any= createElement(clientX, clientY, clientX, clientY);
    setElements((state):any => [...state, newEle]); //copying to the previous state
  };
  const finishDrawing = () => {
    setDrawing(false);
  };
  const draw = (event:any) => {
    ...
    const index: any = elements.length - 1; //last element of the array "elements"
    ...
    const copyElement:any  = [...elements];
